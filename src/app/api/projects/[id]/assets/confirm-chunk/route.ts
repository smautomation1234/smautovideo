...
const schema = z.object({
  path: z.string().min(1),
  content_type: z.enum(["video/mp4", "video/webm", "video/quicktime"])
});
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!input.path.startsWith(`${user.id}/${id}/chunk-`)) {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not confirm chunk upload." }, { status: 400 });
  }
}
